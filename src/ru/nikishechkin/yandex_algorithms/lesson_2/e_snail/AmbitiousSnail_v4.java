 ...
 */
public class AmbitiousSnail_v4 {
        ...
        try (FileReader reader = new FileReader("resources\\lesson2\\inputE_16.txt")) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (delta <= 0) {
                    ...
                    // Завпоминаем индексы с положительной разницей
            ...
            for (int i = 0; i < count_pos + 1; i++) {
                ...
                cur_height += mas[0][index_1[i]];
                if (cur_height > max_height_1) {
                    max_height_1 = cur_height;
                }
                cur_height -= mas[1][index_1[i]];
            }
            ...
            // Положительные разницы в отсортированном порядке по a_i
            // Максимальный a_i для негативной разницы
            // Негативные разницы
            for (int i = 0; i < count_pos; i++) {
                for (int j = 0; j < count_pos; j++) {
                    if (mas[0][indexPos[j]] > mas[0][indexPos[i]]) {
                        temp = indexPos[j];
                        indexPos[j] = indexPos[i];
                        indexPos[i] = temp;
                    }
            ...
            for (int i = 0; i < count_pos + 1; i++) {
                //System.out.println(index[i] + " " + (cur_height + mas[0][index[i]]) + " " + (cur_height + mas[0][index[i]] - mas[0][index[i]]));
                if (i_max_neg == -1 && i == count_pos) continue;
                cur_height += mas[0][index_2[i]];
                if (cur_height > max_height_1) {
                    max_height_2 = cur_height;
                }
                cur_height -= mas[1][index_2[i]];
            }
