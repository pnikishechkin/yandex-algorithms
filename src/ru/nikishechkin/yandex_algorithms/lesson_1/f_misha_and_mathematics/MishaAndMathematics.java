        ...
        try (FileReader reader = new FileReader("resources\\lesson1\\inputF_1.txt")) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (cur == 0) {
                    ...
                    if (!found_odd) {
                        // Все четные, стоящие до первого нечетного - складываем
                        if (cur % 2 == 0) {
                            System.out.print("+");
                        } else {
                            // Нашли нечетное, знак будет определен в зависимости от следующего от него числа
                            // (на следующем шаге цикла)
                            found_odd = true;
                        }
                    } else { // Нечетный был найден, подбираем знак, чтобы сохранить нечетность
                        if (cur == 1 || cur == -1) {
                            System.out.print("x");
                        } else if (cur % 2 == 0) {
                            System.out.print("+"); // Если сложить четное число с нечетным, получится нечетное число
                        } else {
                            System.out.print("x"); // Если нечетное число умножить на нечетное, получится нечетное
                        }
                    }
                }
            }
